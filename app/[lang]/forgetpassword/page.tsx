    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                setTimeout(() => {
                    router.push(`/${langParam}/login`);
                }, 3000);
            } else {
                // This should never happen due to anti-enumeration, but handle gracefully
                setError(data.message || fp.errorOccurred);
            }
        } catch (err) {
            console.error('Forgot password error:', err);
            setError(fp.serverError);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="min-h-screen flex bg-white">
            ...
            <div className="hidden lg:flex lg:w-1/2 relative bg-slate-900">
                ...
                <div className="relative z-20 flex flex-col justify-center px-20 text-white">
                    ...
                    <h1 className="text-5xl font-bold leading-tight mb-4">{fp.heroTitle}</h1>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center px-6 sm:px-12 py-16 bg-slate-50">
                <div className="max-w-md w-full">
                    ...
                    <div className="mb-10 text-center lg:text-left">
                        <h2 className="text-3xl font-extrabold text-slate-900">{fp.pageTitle}</h2>
                        <p className="text-slate-500 mt-2">{fp.pageSubtitle}</p>
                    </div>
                    ...
                    {success && (
                        <div className="bg-green-50/80 border-l-4 border-green-500 text-green-700 p-4 rounded-r-lg text-sm mb-6 flex items-start shadow-sm">
                            ...
                            <div>
                                <p className="font-semibold">{fp.checkEmail}</p>
                                <p className="text-green-600 mt-1">{fp.checkEmailDesc}</p>
                            </div>
                        </div>
                    )}
                    ...
                    {!success ? (
                        <form onSubmit={handleSubmit} className="space-y-6">
                            {/* Email Input */}
                            <div>
                                <label className="block text-sm font-semibold text-slate-700 mb-1">{fp.emailLabel}</label>
                                ...
                                <p className="text-slate-500 text-xs mt-1">{fp.emailHint}</p>
                            </div>
                            ...
                            >
                                ...
                                {loading ? fp.sendingButton : fp.sendButton}
                            </button>
                        </form>
                    ) : null}
                    ...
                    <p className="text-center text-slate-600 mt-8 text-sm">
                        {fp.rememberPassword}{' '}
                        <Link href={`/${langParam}/login`} className="text-blue-600 font-bold hover:underline transition-all">
                            {fp.backToSignIn}
                        </Link>
                    </p>
                </div>
